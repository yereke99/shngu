...
class TrainingWorker(QThread):
    ...
    def run(self):
        try:
            self.progress_update.emit("🔄 Initializing improved trainer...")
            trainer = ImprovedDynamogramTrainer()
            
            self.progress_update.emit("📊 Preparing data with heavy augmentation...")
            train_size, val_size = trainer.prepare_data(augment_factor=15)
            
            if train_size == 0:
                self.training_complete.emit(False, "No training data found. Please add PDF files to data/ folder.")
                return
            
            self.progress_update.emit(f"🏗️  Initializing improved model...")
            trainer.initialize_model()
            
            self.progress_update.emit(f"🎯 Starting improved training ({train_size} train, {val_size} val samples)...")
...
class PredictionWorker(QThread):
    ...
    def run(self):
        try:
            # Use the simple predictor for the improved model
            from predictor_simple import SimpleDynamogramPredictor
            predictor = SimpleDynamogramPredictor()
        ...
        try:
            ...
            if models_dir.exists():
                ...
                if model_files:
                    ...
                    # Initialize predictor with simple version
                    try:
                        from predictor_simple import SimpleDynamogramPredictor
                        self.predictor = SimpleDynamogramPredictor()
                        return True
                    except Exception as e:
